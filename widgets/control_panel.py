...
class ControlPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        # 创建执行选项按钮（使用文本形式避免崩溃）
        self.run_options_button = ToolButton(FluentIcon.ARROW_DOWN)
        self.run_options_button.setFixedSize(60, 60)  # 固定尺寸，高度与执行按钮一致
        self.run_options_button.setToolTip("执行选项")
        self.run_options_button.clicked.connect(self._show_run_options)
        button_layout.addWidget(self.run_options_button, 0)  # 固定宽度，不伸展
    ...
    def _show_run_options(self):
        ...
        显示执行选项弹出框
        """
        pos = self.run_options_button.mapToGlobal(
